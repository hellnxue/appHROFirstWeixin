	...
	$.fn.waterfall = function(options){
		var // 配置信息
			...
			createHtml = $.isFunction(opts.createHtml) ?　
					...
					function(data){
						if(data.goodprice){
							var returnhtm = '<div class="wf_item_inner">' +
								  '<a class="thumb" target="_blank">' +
									'<img class="'+opts.imgClass+'"  src="'+ data.imgSrc +'" />' + '<em class="price">'+ data.price +'</em>' +
								  '</a>' +
								  '<h3 class="title"><a target="_blank">'+ data.title +'</a></h3>' +
								  '<p class="desc">'+ data.describe +'</p>' + '<p class="goodsprice">'+ data.goodprice +'</p>' +
							  '</div>';
						}else{
							var returnhtm = '<div class="wf_item_inner">' +
								  '<a class="thumb" target="_blank">' +
		...
		function showMsg(type){
			switch(type){
				...
				case 'error':
					...
					break;
				case 'finish':
					$wf_result.removeClass('wf_loading').show().html('<div class="none">已加载完毕，没有更多了！</div>');
					isFinish = true;
					break;
			}
		}
